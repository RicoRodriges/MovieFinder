...
export default class TMDBRecommendator {
    ...
    public async recommendMoviesByMovies(movies: Set<Movie>, lang: Language): Promise<Array<[Movie[], Movie]>> {
        const result = await this.recommend([...movies], movie => this.api.getRecommendations(movie.id, lang), movie => movie.id);

        const movieIds = new Set([...movies].map(m => m.id));
        return result.filter(r => !movieIds.has(r[1].id)).map(r => [r[0], r[1].toMovie()]);
    }

    public async recommendMoviesByActors(people: Set<Person>, lang: Language): Promise<Array<[Person[], Movie]>> {
        const result = await this.recommend([...people], actor => this.api.searchMoviesByPerson(actor.id, lang), movie => movie.id);
