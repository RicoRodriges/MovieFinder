    ...
    <div>
        ...
        >
            <template v-slot:default="val">
                ...
                <div class="option__desc d-inline-block align-middle">
                    ...
                    <div class="option__small text-muted">
                        {{ $t("general.popularity") }}: {{ val.item.popularity }}
                    </div>
                </div>
            </template>
        </ItemSelector>
        <div class="my-1 text-center">
            <button
                v-if="value.length > 0"
                type="button"
                class="btn btn-danger d-inline-block mx-2"
                @click="onClear"
            >
                {{ $t("actor.clearList") }}
            </button>
            <button
                v-if="setActions && actorSet.length > 0"
                type="button"
                class="btn btn-success d-inline-block mx-2"
                @click="onAppendFromSet"
            >
                {{ $t("actor.appendToList") }}
            </button>
            <button
                v-if="setActions && value.length > 0"
                type="button"
                class="btn btn-success d-inline-block mx-2"
                @click="onAppendToSet"
            >
...
export default class ActorSelector extends Vue {
    ...
    private async suggest(text: string): Promise<Person[]> {
      ...
      return (await this.api.searchPerson(text, this.lang)).sort(
