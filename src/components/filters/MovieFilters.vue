    ...
    <div class="row">
        <div class="col-md-6 col-12">
            <div class="card mx-auto my-2" style="max-width: 40rem;">
                <div class="card-body">
                    <div class="card-title"><img src="@/assets/filter.svg" class="mx-2" style="height: 1.8rem;"> {{ $t('filter.filters') }}</div>
                    <div class="card-text">
                        <button type="button"
                                class="btn btn-outline-danger my-3" :class="{'active': !excludeMoviesFromSet}"
                                @click="excludeMoviesFromSet = !excludeMoviesFromSet">
                            {{ excludeMoviesFromSet ? $t('movie.showAll') : $t('movie.excludeMovies') }}
                        </button>
                        <br>
                        <TagSelector v-model="excludedGenres" :items="genres" :placeholder="$t('filter.chooseGenresToExclude')" :toName="genreToName"/>
                        <br>
        ...
        <div class="col-md-6 col-12">
            <div class="card mx-auto my-2" style="max-width: 40rem;">
                <div class="card-body">
                    <div class="card-title"><img src="@/assets/sort.svg" class="mx-2" style="height: 1.8rem;"> {{$t('filter.sort')}}</div>
                    <div class="card-text">
                        {{$t('filter.sortBy')}}
                        <select v-model="sort" class="form-control form-control-sm d-inline ml-2" style="max-width: 10rem;">
                            <option value="" disabled>{{$t('general.chooseOptions')}}</option>
                            <option v-for="o in sortOptions" :key="o.id" :value="o.id" :selected="o.id === sort">{{$t(o.n)}}</option>
                        </select>
                    </div>
                </div>
            </div>
