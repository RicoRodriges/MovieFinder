...
export default class TMDBApi {
    ...
    protected static readonly max_similar_pages = 4;
    ...
    public async getPerson(personId: PersonId, language: Language): Promise<TMDBPerson | undefined> {
    ...
    public async getMovie(movieId: MovieId, language: Language): Promise<TMDBMovie | undefined> {
        const response = await this._getRequest(`${TMDBApi.apiHost}/movie/${movieId}`, {
            api_key: TMDBApi.apiKey,
            language,
        });
        if (response.id !== undefined) {
    ...
    public async getRecommendations(movieId: MovieId, language: Language): Promise<TMDBMovie[]> {
        const request = (page: number) => this._getRequest(`${TMDBApi.apiHost}/movie/${movieId}/recommendations`, {
            api_key: TMDBApi.apiKey,
            language,
            page: page.toString(),
        });
        const totalPages = (r: any) => r.total_pages;
        const map = (r: any) => r.results.map(async (r: any) => this.responseToMovie(r, language));
        return this._fetchAllPages(request, totalPages, map);
    }

    public async searchMoviesByPerson(personId: PersonId, language: Language): Promise<TMDBMovie[]> {
    ...
    public async getGenres(language: Language): Promise<Map<GenreId, TMDBGenre>> {
        ...
        if (!r) {
            r = this._getGenres(language);
            this.genres.set(language, r);
        }
        return r;
    }

    private async _getGenres(language: Language): Promise<Map<GenreId, TMDBGenre>> {
    ...
    private async _fetchAllPages<R, T>(
        ...
        if (totalPages(response) > page && (pageLimit === undefined || pageLimit >= page)) {
            nextPages = this._fetchAllPages(request, totalPages, getItems, pageLimit, page + 1);
        } else {
